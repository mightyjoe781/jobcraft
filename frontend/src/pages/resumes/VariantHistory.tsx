...
export default function VariantHistory() {
  ...
  async function handleForkAsBase(v: Variant) {
    ...
    await resumeApi.createBaseResume({
      ...
      tex_source: "", // will be filled from variant tex — simplified for now
    });
    alert("Forked — go to My Resumes to open it");
  }

  return (
    <div className="p-8">
      <h1 className="text-2xl font-bold text-white mb-1">Variant History</h1>
      <p className="text-gray-400 text-sm mb-6">All AI-tailored outputs from your base resumes.</p>
        ...
        <div className="overflow-hidden rounded-xl border border-gray-800">
          <table className="w-full text-sm">
            <thead className="bg-gray-900 text-gray-400 text-xs uppercase tracking-wide">
              <tr>
                <th className="px-4 py-3 text-left">Label / Job</th>
            ...
            <tbody className="divide-y divide-gray-800">
              {variants.map((v) => (
                <tr key={v.id} className="bg-gray-900 hover:bg-gray-800/50 transition-colors">
              ...
              {diffLoading ? (
                ...
                <>
                  <div className="flex-1 overflow-auto p-4 border-r border-gray-800">
                    <p className="text-xs text-gray-500 mb-2 uppercase">Original</p>
                    <pre className="text-xs text-gray-300 whitespace-pre-wrap font-mono leading-5">
                      {diff.original_tex}
                    </pre>
                  </div>
                  <div className="flex-1 overflow-auto p-4">
                    <p className="text-xs text-gray-500 mb-2 uppercase">Modified</p>
                    <pre className="text-xs text-gray-300 whitespace-pre-wrap font-mono leading-5">
                      {diff.modified_tex}
                    </pre>
                  </div>
